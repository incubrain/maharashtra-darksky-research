...
def generate_all_district_reports(yearly_df, trends_df, stability_df, gdf,
                                  output_dir):
    """Generate a single consolidated PDF atlas with all district reports."""
    os.makedirs(output_dir, exist_ok=True)
    atlas_path = os.path.join(output_dir, "district_atlas.pdf")

    districts = sorted(trends_df["district"].unique())

    with PdfPages(atlas_path) as pdf:
        for district in districts:
            _write_district_pages(
                pdf, district, yearly_df, trends_df, stability_df, gdf,
            )

    log.info("Generated district atlas (%d districts): %s",
             len(districts), atlas_path)


def _write_district_pages(pdf, district_name, yearly_df, trends_df,
                          stability_df, gdf):
    """Write pages for a single district into the shared PdfPages object."""
    sub = yearly_df[yearly_df["district"] == district_name].sort_values("year")
    trend_row = trends_df[trends_df["district"] == district_name]
    stab_row = stability_df[stability_df["district"] == district_name] if stability_df is not None else None
    ...
    if not trend_row.empty:
        ...
        def _fmt(val, fmt_str, fallback="N/A"):
            """Format a value, returning fallback for None/NaN."""
            if val is None:
                return fallback
            try:
        ...
        lines += [
            f"Latest Median Radiance: {_fmt(r.get('median_radiance_latest'), '.2f')} nW/cm\u00b2/sr",
            f"ALAN Classification: {r.get('alan_class') or 'N/A'}",
            f"Annual % Change: {_fmt(r.get('annual_pct_change'), '+.2f')}%",
            f"95% CI: [{_fmt(r.get('ci_low'), '+.2f')}%, {_fmt(r.get('ci_high'), '+.2f')}%]",
            f"R\u00b2: {_fmt(r.get('r_squared'), '.3f')}",
            f"p-value: {_fmt(r.get('p_value'), '.2e')}",
            f"Years of data: {r.get('n_years') or 'N/A'}",
            "",
        ]
    if stab_row is not None and not stab_row.empty:
        s = stab_row.iloc[0]
    ...
    if len(sub) >= 2:
        fig, ax = plt.subplots(figsize=(10, 6))
        years = sub["year"].values.astype(float)
        radiance = pd.to_numeric(sub["median_radiance"], errors="coerce").fillna(0.0).values

...
def generate_all_site_reports(all_site_data, yearly_df, output_dir,
    ...
    for site_name in all_site_data["name"].unique():
        ...
        generate_site_report(
            site_name=site_name,
            site_type=site_row.get("type", "site"),
            metrics_df=all_site_data,
            yearly_df=yearly_df,
            buffer_comparison_df=buffer_comparison_df,
            directional_df=directional_df,
            proximity_df=proximity_df,
            output_dir=report_dir,
        )
    log.info("Generated %d site reports in %s",
             all_site_data["name"].nunique(), report_dir)

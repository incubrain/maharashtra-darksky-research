...
def generate_site_maps(gdf_sites, metrics_df, district_gdf, subset_dir, output_dir,
    ...
    try:
        ...
        pass  # labels stay at centroid if adjustText not installed
...
def generate_site_timeseries(yearly_df, output_dir):
    ...
    Only generates a plot if the relevant type has data in yearly_df.
    For cities with many entries, shows top-N by latest radiance to
    ...
    # ── Cities time series ────────────────────────────────────────────
    city_names = yearly_df[yearly_df["type"] == "city"]["name"].unique()
    if len(city_names) > 0:
        # Show top 15 cities by latest-year radiance to keep legend readable
        max_legend = 15
        if len(city_names) > max_legend:
            latest_year = yearly_df["year"].max()
            latest = yearly_df[(yearly_df["type"] == "city") & (yearly_df["year"] == latest_year)]
            top_names = latest.nlargest(max_legend, "median_radiance")["name"].tolist()
        else:
            top_names = list(city_names)
        ...
        n_shown = len(top_names)
        title_suffix = f" (top {n_shown} by radiance)" if n_shown < len(city_names) else ""
        ax.set_title(f"ALAN Time Series: Cities{title_suffix}", fontsize=14)
        ...
        log.info("Skipping cities time series — no city data in yearly_df")

    # ── Dark-sky sites time series ────────────────────────────────────
    site_names = yearly_df[yearly_df["type"] == "site"]["name"].unique()
    if len(site_names) > 0:
        fig, ax = plt.subplots(figsize=(14, 8))
        ...
        log.info("Skipping dark-sky time series — no site data in yearly_df")

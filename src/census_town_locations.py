...
compatible with config.URBAN_CITIES format, allowing the city pipeline
to use census-derived town lists instead of the hand-picked 43 cities.

Usage:
    from src.census_town_locations import load_census_town_locations

    # Returns {town_name: {"lat": ..., "lon": ..., "district": ...}}
...
from src.datasets._name_resolver import normalize_name, resolve_names
...
def _title_case_town(norm_name: str) -> str:
    ...
    # Handle multi-word names with proper capitalisation
...
def load_census_town_locations(
    ...
    ----------
    data_dir : str
        Directory containing census_towns_geocoded.csv.
    exclude_failed : bool
        If True, exclude towns where geocoding failed.
    vnl_district_names : list[str], optional
        District names from VNL shapefile for name resolution.
    ...
    # Resolve district names if VNL names provided
    ...
    # Handle duplicate town names across districts
    ...
    for _, row in df.iterrows():
        ...
        # Disambiguate duplicate names by appending district
        if norm in duplicated_names:
            # Check if it's actually in different districts
            same_name = df[df["town_name"] == norm]
            if same_name["district"].nunique() > 1:
                display = f"{display} ({row['district']})"

        # If still duplicated (same name, same district), keep largest population
